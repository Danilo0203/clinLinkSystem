...
const AppointmentBooking = () => {
  ...
  useEffect(() => {
    // Fetch specialties
    axios.get('https://clinclink-bf4bg.ondigitalocean.app/api/specialization')
    ...
    if (selectedAppointment && newAppointmentTime) {
      ...
      updateAppointment(selectedAppointment.id, updatedAppointment)
        .then(response => {
          ...
          setBookedAppointments(bookedAppointments.map(appointment => 
  ...
  return (
    <div>
      ...
      <DataTable value={availableAppointments} selectionMode="single" onSelectionChange={e => setSelectedAppointment(e.value)}>
        ...
        <Column field="availableTime" header="Horario Disponible" />
      </DataTable>
        ...
        <Column 
          body={(rowData: Appointment) => (
            <div>
              ...
              <Button label="Reprogramar" onClick={() => { setSelectedAppointment(rowData); setReprogramDialogVisible(true); }} />
            </div>
          )}
          header="Acciones"
        />
